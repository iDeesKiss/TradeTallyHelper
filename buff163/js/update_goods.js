...
function goods_lots() {
    ...
    for (var lot of lots) {
        if (isFloat) {
            // Меняет цвет в зависимости от флота
            // const float = Number(get_float(lots[key].querySelector(".wear-value").textContent))
            // lots[key].querySelector(".wear-value").style.color = filt_getFloatColor(wear, float)

            //Добавление div paintseed под float
            seed = addPaintseed(lot)
            lot.querySelector(".wear-value").closest(':not(.wear-value)').appendChild(seed)

            //Добавление div last inspect
            // let last_inspect = addLastInspect(lots[key])
            // lots[key].querySelector(".wear-value").closest(':not(.wear-value)').appendChild(last_inspect)
